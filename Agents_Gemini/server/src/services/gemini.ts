...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o áudio para português do Brasil. Seja preciso e naturual na transcrição. Matenha a pontuação adequada e divida o texto em parágrafos quando for apropriado'
            },
            {
                inlineData: {
                    mimeType,
...
export async function generateEmbeddings(text: string) {
    const response = await gemini.models.embedContent({
        ...
        contents: [{ text }],
        config: {
            taskType: 'RETRIEVAL_DOCUMENT', //Estou falando que vou fazer busca semanticas e utilizar o conteudo em outros prompts
        }
    })
