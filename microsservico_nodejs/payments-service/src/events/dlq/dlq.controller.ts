...
import { DlqService, DQLMessage, DQLStats } from './dlq.service';
...
export class DlqController {
  ...
  /**
   * GET /dlq/stats
   * Retorna estatísticas da DQL
   */
  @Get('stats')
  async getStatus(): Promise<DQLStats> {
    try {
  ...
  ): Promise<{ count: number; messages: DQLMessage[] }> {
    try {
   ...
   * Reprocessa todas as mensagens na DLQ (uma por uma)
   */
  @Post('reprocess-all')
   ...
   * Remove uma mensagem especifica da DLQ (descarta permanentemente)
   */
